'use client';

import { useState } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Plus, ArrowUpRight, ArrowDownRight, CreditCard, Wallet, TrendingUp, X } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useTheme } from '@/lib/ThemeContext';
import { THEMES } from '@/lib/themes';
import { useUser } from '@/hooks/useUser';
import { useActionContext } from '@/context/ActionContext';

export function SpeedDial() {
  const [isOpen, setIsOpen] = useState(false);
  const { theme } = useTheme();
  const { userData, loading } = useUser();
  const { openAction } = useActionContext();
  const colors = THEMES[theme] || THEMES.ORBITA;

  if (loading || !userData) return null;

  const actions = [
    {
      icon: ArrowUpRight,
      label: 'A Pagar',
      color: 'bg-red-500',
      action: () => {
        openAction('contas_pagar');
        setIsOpen(false);
      }
    },
    {
      icon: ArrowDownRight,
      label: 'A Receber',
      color: 'bg-emerald-500',
      action: () => {
        openAction('contas_receber');
        setIsOpen(false);
      }
    },
    {
      icon: CreditCard,
      label: 'Cartão',
      color: 'bg-purple-500',
      action: () => {
        openAction('fatura');
        setIsOpen(false);
      }
    },
    {
      icon: TrendingUp,
      label: 'Comprar',
      color: 'bg-blue-500',
      action: () => {
        openAction('investimento_compra');
        setIsOpen(false);
      }
    },
    {
      icon: Wallet,
      label: 'Transação',
      color: 'bg-amber-500',
      action: () => {
        openAction('despesa');
        setIsOpen(false);
      }
    },
  ];

  const handleAction = (action: any) => {
    action.action();
  };

  return (
    <>
      <div className="fixed bottom-24 right-6 z-50 flex flex-col items-end gap-4">
        <AnimatePresence>
          {isOpen && (
            <div className="flex flex-col items-end gap-3 mb-2">
              {actions.map((action, i) => (
                <div key={action.label} className="flex flex-col items-end gap-1">
                  <motion.div
                    initial={{ opacity: 0, scale: 0, y: 20 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0, y: 20 }}
                    transition={{ delay: (actions.length - i) * 0.05 }}
                    className="flex items-center gap-3 group"
                  >
                    <button
                      onClick={() => handleAction(action)}
                      className={cn("w-12 h-12 rounded-2xl flex items-center justify-center text-[var(--color-bg-dark)] shadow-lg transition-transform active:scale-95 medieval-glow", action.color)}
                    >
                      <action.icon className="w-6 h-6" />
                    </button>
                  </motion.div>
                </div>
              ))}
            </div>
          )}
        </AnimatePresence>

        <button
          onClick={() => setIsOpen(!isOpen)}
          className={cn(
            "w-14 h-14 rounded-[2rem] flex items-center justify-center text-[var(--color-bg-dark)] shadow-2xl transition-all active:scale-95 medieval-glow",
            isOpen ? "bg-[var(--color-bg-panel)] text-[var(--color-text-main)] rotate-45 border border-[var(--color-border)]" : "bg-[var(--color-primary)]"
          )}
        >
          {isOpen ? <X className="w-8 h-8 -rotate-45" /> : <Plus className="w-8 h-8" />}
        </button>
      </div>
    </>
  );
}
